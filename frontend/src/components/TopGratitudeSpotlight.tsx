...
import { Trophy, Star, Heart, Tag, Sparkles, X, ChevronRight, Award, Crown, Flame } from 'lucide-react';
...
export const TopGratitudeSpotlight: React.FC = () => {
  ...
  const getRankBadge = (rank: number) => {
    switch (rank) {
      case 1:
        return {
          bg: 'bg-amber-100 text-amber-800 border-amber-300 ring-amber-400/30',
      ...
      default:
        return {
          ...
          label: `${rank}th`,
          gradient: 'from-[#0058bd] to-blue-400',
        };
    }
  };
      ...
      >
        ...
        <div className="flex items-center gap-3 ml-auto shrink-0">
          ...
          <div className="flex items-center gap-2 bg-white/90 border border-amber-200/70 px-3 py-1.5 rounded-xl shadow-2xs">
            <div className="flex items-center gap-1 text-xs font-bold text-amber-800" title="Appreciation Score">
              <Flame className="w-3.5 h-3.5 text-amber-500 fill-amber-400" />
              <span>{champion.score} pts</span>
            </div>
            <div className="h-3 w-px bg-slate-200" />
            <div className="flex items-center gap-1 text-[11px] font-semibold text-rose-600" title="Likes Received">
              <Heart className="w-3 h-3 fill-rose-500 text-rose-500" />
              <span>{champion.likesCount}</span>
            </div>
            <div className="flex items-center gap-1 text-[11px] font-semibold text-[#0058bd]" title="Notes Tagged In">
              <Tag className="w-3 h-3" />
              <span>{champion.tagsCount}</span>
            </div>
          </div>
            ...
            <div className="hidden md:flex items-center -space-x-2">
              {runnersUp.map((m) => (
                <div
                  ...
                  title={`${m.rank}. ${m.user.fullName} (${m.score} pts)`}
                >
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-black/5 bg-gradient-to-r from-amber-500/10 via-amber-100/40 to-white">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-lg sm:text-xl font-extrabold text-[#191c1d]">Top Appreciated Members</h3>
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-200 text-amber-900 border border-amber-300/50">
                        Top 5
                      </span>
                    </div>
                    <p className="text-xs text-slate-500">Ranked by gratitude tags received & likes on their appreciation notes</p>
                  </div>
                </div>
                <button
                  onClick={() => setIsLeaderboardOpen(false)}
              ...
              <div className="px-6 pt-6 pb-2 bg-gradient-to-b from-amber-50/40 to-transparent">
                <div className="grid grid-cols-3 gap-2 sm:gap-4 items-end max-w-md mx-auto text-center">
                  {/* Rank 2 (Silver) */}
                  {topMembers[1] && (
                    <div
                      onClick={() => handleMemberClick(topMembers[1].user.fullName)}
                      className="flex flex-col items-center p-3 rounded-2xl bg-white/80 border border-slate-200/80 shadow-2xs hover:shadow-md transition-all cursor-pointer group"
                    >
                      ...
                      <span className="text-[10px] text-amber-700 font-extrabold mt-0.5">
                  ...
                  {champion && (
                    <div
                      ...
                      className="flex flex-col items-center p-3.5 sm:p-4 rounded-2xl bg-gradient-to-b from-amber-100/90 to-amber-50/80 border-2 border-amber-400 shadow-md hover:shadow-lg transition-all cursor-pointer group -translate-y-2"
                    >
                      <div className="relative mb-2">
                        <Crown className="w-5 h-5 text-amber-500 fill-amber-400 absolute -top-4 left-1/2 -translate-x-1/2 animate-bounce" />
                        <div
                          className="w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-base shadow-lg ring-4 ring-amber-300/60"
                      ...
                      <span className="text-[11px] text-amber-900 font-black mt-0.5 flex items-center gap-1">
                  ...
                  {topMembers[2] && (
                    <div
                      ...
                      className="flex flex-col items-center p-3 rounded-2xl bg-white/80 border border-amber-700/20 shadow-2xs hover:shadow-md transition-all cursor-pointer group"
                    >
                      <div className="relative mb-2">
                        <div
                          className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-md"
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between p-3 rounded-2xl transition-all cursor-pointer border ${
                        member.rank === 1
                          ? 'bg-amber-50/70 border-amber-300 shadow-xs hover:bg-amber-100/60'
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="flex items-center gap-2">
                          <span
                            className="flex items-center gap-1 text-xs font-semibold text-rose-600 bg-rose-50 px-2 py-1 rounded-lg border border-rose-100"
                            title={`${member.likesCount} total likes received on appreciation notes`}
                          >
                            <Heart className="w-3.5 h-3.5 fill-current" />
                            <span>{member.likesCount}</span>
                          </span>
                          <span
                            className="flex items-center gap-1 text-xs font-semibold text-[#0058bd] bg-blue-50 px-2 py-1 rounded-lg border border-blue-100"
                            title={`${member.tagsCount} gratitude notes tagged in`}
                          >
                            <Tag className="w-3.5 h-3.5" />
                            <span>{member.tagsCount}</span>
                          </span>
                        </div>
                        ...
                        <div
                          ...
                          title="Total Appreciation Score"
                        >
              ...
              <div className="px-6 py-3.5 bg-white border-t border-black/5 flex items-center justify-between">
                <p className="text-[11px] text-slate-400">💡 Click any member to filter the wall by their notes</p>
                <button
                  onClick={() => setIsLeaderboardOpen(false)}
                  className="px-5 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs transition-colors cursor-pointer"
                >
                  Close
                </button>
              </div>

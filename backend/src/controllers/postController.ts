...
import { createPost, getWallPosts, toggleLikePost, reportPost } from '../services/postService';
...
export const toggleLikeHandler = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (!req.user?.userId) {
      res.status(401).json({ success: false, message: 'Must be logged in to like a post' });
      return;
    }

    const postId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const result = await toggleLikePost(postId, req.user.userId);
